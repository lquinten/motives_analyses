...
flextable(results, col_keys=c("Proclivity", "p_lr_total_0.01", "b_intercept_0.01", "CI_intercept", "p_lr_intercept_0.01", "b_predicted_class_1_0.01", "CI_class1", "p_lr_predicted_class_1_0.01", "b_predicted_class_2_0.01", "CI_class2", "p_lr_predicted_class_2_0.01")) %>% 
  ...
  autofit()
```
*Note.* Global LRT shows the *p* value of a test of the full against an intercept-only model. Penalised LRTs were used to obtain p values. Confidence intervals were computed using the Wald’s test statistics.
...
lapply(plot_results$Proclivity %>% unique(), function(x){
  ggplot(plot_results %>% filter(Proclivity == x), aes(x=class, y=odds)) +
  geom_bar(stat="identity", position="dodge") +
  ylim(0,1)
  })
