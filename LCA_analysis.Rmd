...
```{r setup & library, include=FALSE, message=F, warning=F}
...
#A 2 represents a male. We want to only grab males. 
...
#We refrain from renaming the variables as I have done before to make interpretation across analyses easier. 
```

The distributions of some of the variables are very skewed, and some categories are only inhabited by a few participants (see plots below).

Distributions male participants:

```{r violin plots, echo=F, warning=F, fig.width=12}
...
Therefore, we decided to split the scales into quantiles (see plots), and, when sufficient data to split the data into quantiles was not available, into dummy coded variables. 
...
#Create a dataframe with modal weights
...
We proceed by estimating Firths logistic regression.
