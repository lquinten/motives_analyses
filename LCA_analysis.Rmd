...
# Variable <- c("Sexual interest in children", "CSBD-7", "LON", "Mating effort", "Mating value", "PPCS-6", "Sex drive", "Social anxiety")
...
#Create vector with rownames
Scale <- c("Sexual interest in children", "CSBD-7", "LON", "Mating effort", "Mating value", "PPCS-6", "Sex drive", "Social anxiety")
#Bind Variable column to the beginning
...
### Scree Plot Females
```{r scree_plot_f, eval=F}
plot(lca_fit_f)
```

### Class Proportions Females
```{r class_proportion_f, eval=F}
#class proportions
...
```{r prob_plot_f, eval=F, fig.width=12}
plot_prob(lca_final_model_f)
```
-->

# __Distal Outcomes Analyses__
...
flextable(results, col_keys=c("Proclivity", "p_lr_total_0.01", "b_intercept_0.01", "CI_intercept", "p_lr_intercept_0.01", "b_predicted_class_1_0.01", "CI_class1", "p_lr_predicted_class_1_0.01", "b_predicted_class_2_0.01", "CI_class2", "p_lr_predicted_class_2_0.01")) %>% 
  ...
  autofit()
```
*Note.* Global LRT shows the *p* value of a test of the full against an intercept-only model.
