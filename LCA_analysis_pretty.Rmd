...
Descriptives for the newly categorised scales are shown in Table X.
```{r factor, eval=F}
...
For class enumeration, the Bayesian information criterion (BIC) is, according to simulation studies, the most reliable measure [@van_lissa_recommended_2023; @masyn_latent_2013]. It reached a minimum at three classes (see Figure Y). Following Van Lissa et al. (2023), a likelihood ratio test (LRT) was not calculated to avoid biased results due to unmet conditions. Instead, fit indices were inspected to assess how well models with 1-7 classes perform in that data set.
