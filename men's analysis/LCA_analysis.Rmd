...
data_males <- haven::read_spss("./data/dataE_2112.sav") %>% dplyr::select(gender, CASE, sexdrive2, CSBD, PPCS_6, meffort, socialanx, lon, mvalue, attr) %>% filter(gender == 2) %>% dplyr::select(-gender)
```
...
flextable(prob_table_LCA_m) %>%
  ...
  set_header_labels(Probability.class1="Class 1: \"Mating prowess\"", Probability.class2="Class 2: \"Social needs\"", Probability.class3="Class 3: \"Multiple motivations\"") %>%
  theme_booktabs() %>%
  autofit()
```
...
BCH_expanded_dataframe <- BCH_expanded_dataframe %>% full_join(y=haven::read_spss("./data/data raw/dataE_2112.sav") %>% filter(gender == 2) %>% select(CASE, starts_with("Y_") & ends_with("r")) %>% mutate(CASE = CASE %>% as.numeric()),by="CASE")
```
...
CASE_yp <-  haven::read_spss("./data/dataE_2112.sav") %>% filter(gender == 2)  %>% filter(range_young1 < 15) %>% dplyr::select(CASE) %>% unlist()
...
ggplot(plot_results, aes(x=Proclivity, y=Odds, group=as.factor(class), linetype=as.factor(class))) +
  ...
  scale_linetype_manual(name=NULL, values=c("dotted", "dashed", "solid"), labels=c("Class 1: \"Mating prowess\"", "Class 2: \"Social needs\"", "Class 3: \"Multiple motivations\"")) +
  scale_shape_manual(name=NULL, values=c(3, 1, 0), labels=c("Class 1: \"Mating prowess\"", "Class 2: \"Social needs\"", "Class 3: \"Multiple motivations\"")) +
  theme_classic() +
